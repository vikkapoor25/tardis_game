...
class Explanation {
  ...
  static async getAllExplanations() {
    const response = await db.query('SELECT q.question_id, q.answer, s.category, q.explanation FROM questions q LEFT JOIN scenarios s ON (q.scenario_id = s.scenario_id)')
